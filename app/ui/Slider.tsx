...
const thumbStyles = compose(
  ...
  cva({
    base: [
      ...
      // vertical
      "group-data-[orientation=horizontal]:mt-4 group-data[orientation=vertical]:ml-3",
    ],
    variants: {
      isDragging: {
        true: "shadow-lg",
...
function Slider<T extends number | number[]>({
  ...
  return (
    ...
    >
      <Label>{label}</Label>
      <SliderOutput className="text-xs tabular-nums text-neutral-text group-data-[disabled]:text-neutral-placeholder data-[orientation=vertical]:hidden">
        {({ state }) =>
