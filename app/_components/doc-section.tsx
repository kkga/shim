...
function Demo({ title, demo, code, className, children }: Props) {
  return (
    <section className="col-span-full grid grid-cols-subgrid items-start gap-y-6 md:gap-y-0">
      <div className="border-neutral-4 flex flex-col gap-4 border-t pt-4 text-[15px] leading-6 lg:gap-6">
        {(title || children) && (
          <div className="*:last:mb-0">
            {title && (
              <h2 className="text-neutral-text-contrast mb-1 text-balance font-medium">
                {title}
              </h2>
            )}
            {children}
